<template>
  <div class="orchestration-page">
    ...
    <div class="orchestration-container">
      ...
      <div v-if="result" class="result-container"><h3>📋 Result</h3><pre>{{ result }}</pre></div>
    </div>
  </div>
</template>
<script>
...
export default {
  data() { return { agents: [], selectedAgents: [], task: '', result: null, loading: false }; },
  mounted() { this.fetchAgents(); },
  methods: {
    async fetchAgents() { try { const { data } = await api.get('/agents'); this.agents = data.filter(a => a.status === 'active'); } catch(e) { console.error(e); } },
    async orchestrate() {
      if (!this.task || this.selectedAgents.length === 0) return;
      this.loading = true; this.result = null;
      try {
        const { data } = await api.post('/orchestrate', { task: this.task, agent_ids: this.selectedAgents });
...
.orchestration-container { display: flex; flex-direction: column; gap: 1.5rem; margin-top: 1.5rem; }
.task-input textarea { width: 100%; padding: 1rem; border: 1px solid var(--border); border-radius: 12px; background: var(--bg-card); color: var(--text-primary); }
...
.btn-orchestrate { background: linear-gradient(135deg, #8b5cf6, #6d28d9); color: white; border: none; padding: 0.8rem 2rem; border-radius: 12px; font-size: 1.1rem; font-weight: 600; cursor: pointer; }
