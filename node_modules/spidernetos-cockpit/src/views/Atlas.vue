<template>
  <div class="atlas-container">
    <div class="page-header">
      <h1> Atlas  General AI Assistant</h1>
    ...
    <div class="chat-wrapper">
      <div class="chat-messages" ref="messagesContainer">
        ...
        <div v-if="loading" class="message assistant"><div class="avatar"></div><div class="bubble typing">...</div></div>
      </div>
      <div class="chat-input-area">
        ...
        <div class="input-group">
          <input v-model="inputMessage" @keyup.enter="sendMessage" placeholder='Try "send email to email@example.com about subject"' />
...
export default {
  data() {
    return {
      ...
      inputMessage: '',
      loading: false,
      suggestions: ['create agent SalesBot', 'send email to test@example.com about Hello', '/agents', '/help']
    }
  },
  mounted() {
    this.messages.push({
...
**Try these commands:**
 ...
 \`send email to email@example.com about subject\`
 ...
 \`/help\``
    ...
    async sendMessage() {
      ...
      try {
        // Try the intent endpoint first
        const response = await api.post('/atlas/intent', { message: command })
        ...
        try {
          const fallback = await api.post('/atlas/chat', { message: command })
          this.messages.push({ id: Date.now(), role: 'assistant', content: fallback.data.response || fallback.data.message || 'No response' })
        } catch(e) {
