...
JSON response:"""
    ...
    try:
        response = query_ollama(prompt, PRIMARY_MODEL, timeout=30)
...
def generate_response(message: str, intent: Dict[str, Any], context: Dict) -> str:
    ...
    model = PRIMARY_MODEL if complexity in ["medium", "high"] else FALLBACK_MODEL

    prompt = f"""You are {agent_name}, an AI agent in SpiderNetOS. 
