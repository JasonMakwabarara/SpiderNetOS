  ...
  <div class="security-page">
    ...
    <div class="security-grid">
      ...
      <div class="security-card">
        ...
        <button @click="toggle2FA" class="btn-security" :class="twoFactorEnabled ? 'btn-danger' : 'btn-success'">
  ...
  methods: {
    ...
    async toggle2FA() {
      try {
        if (this.twoFactorEnabled) {
          ...
          alert('✅ 2FA enabled! Secret: ' + data.secret)
        }
      } catch (e) {
    ...
    async fetchAuditLogs() {
      try {
        ...
        this.auditLogs = data.slice(0, 10)
      } catch (e) {
...
.page-header h1 { font-size: 2rem; font-weight: 700; background: linear-gradient(135deg, #ef4444, #dc2626); -webkit-background-clip: text; background-clip: text; color: transparent; }
.security-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 1.5rem; }
...
.btn-security { padding: 0.6rem 1.5rem; border: none; border-radius: 12px; cursor: pointer; font-weight: 600; transition: all 0.2s; }
